...
const C = {
  bg:"#07090f",surface:"#0d1018",card:"#121620",border:"#1a2030",
  accent:"#00d4ff",accentBg:"#00d4ff0f",accentBorder:"#00d4ff25",
  gold:"#f0b429",goldBg:"#f0b4290f",goldBorder:"#f0b42925",
  green:"#10b981",greenBg:"#10b9810f",red:"#f43f5e",redBg:"#f43f5e0f",
  text:"#dde4f0",sub:"#5c6580",
...
body{background:#07090f;color:#dde4f0;font-family:'Outfit',sans-serif;min-height:100vh}
...
const Spinner = ({ size=16, light }) => (
  <span style={{width:size,height:size,border:`2px solid ${light?"#ffffff30":"#1a2030"}`,borderTopColor:light?"#fff":"#00d4ff",borderRadius:"50%",display:"inline-block",animation:"spin .65s linear infinite",flexShrink:0}}/>
);

const Toast = ({ msg, type, onClose }) => {
  useEffect(() => { const t = setTimeout(onClose, 4500); return ()=>clearTimeout(t); }, [onClose]);
  const col = type==="error"?"#f43f5e":type==="success"?"#10b981":"#00d4ff";
  return (
...
const Btn = ({ children, onClick, loading, variant="primary", full, small, disabled, style={} }) => {
  const vs = {
    ...
    danger:{background:C.redBg,color:C.red,border:`1px solid #f43f5e30`},
  };
  return (
    <button onClick={onClick} disabled={!!loading||!!disabled}
      style={{display:"inline-flex",alignItems:"center",justifyContent:"center",gap:7,borderRadius:9,border:"none",cursor:loading||disabled?"not-allowed":"pointer",fontWeight:600,fontSize:small?12:14,padding:small?"7px 15px":"11px 22px",transition:"all .15s",width:full?"100%":undefined,opacity:loading||disabled?.65:1,...vs[variant],...style}}>
...
const Input = ({ label, error, ...props }) => (
  <div style={{display:"flex",flexDirection:"column",gap:5}}>
    ...
    <input {...props} style={{background:C.surface,border:`1px solid ${error?C.red:C.border}`,borderRadius:8,padding:"10px 13px",color:C.text,fontSize:14,outline:"none",...props.style}}
...
const Badge = ({ label, color=C.sub }) => (
  <span style={{background:`${color}18`,color,border:`1px solid ${color}28`,padding:"2px 9px",borderRadius:20,fontSize:11,fontWeight:600,textTransform:"capitalize",fontFamily:"'Fira Code',monospace",whiteSpace:"nowrap"}}>
    {label}
  </span>
);
...
const CopyBtn = ({ text, label }) => {
  ...
  return (
    <button onClick={copy} style={{background:ok?C.accentBg:C.surface,border:`1px solid ${ok?C.accent:C.border}`,borderRadius:7,padding:"7px 13px",color:ok?C.accent:C.sub,cursor:"pointer",fontSize:12,fontFamily:"'Fira Code',monospace",transition:"all .18s",whiteSpace:"nowrap"}}>
...
const Table = ({ heads, rows }) => (
    ...
    <table style={{width:"100%",borderCollapse:"collapse",minWidth:400}}>
      <thead>
        <tr style={{background:C.surface}}>
          {heads.map(h=>(
            <th key={h} style={{padding:"10px 14px",textAlign:"left",fontSize:10,color:C.sub,fontFamily:"'Fira Code',monospace",borderBottom:`1px solid ${C.border}`,whiteSpace:"nowrap"}}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
  ...
  const submit = async () => {
    ...
    try {
      let data;
      if (mode==="login") {
        data = await req("/api/auth/login",{method:"POST",body:JSON.stringify({email:form.email,password:form.password})});
      } else {
  ...
  return (
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:`radial-gradient(ellipse 80% 40% at 50% -5%,#00d4ff15,transparent 60%),#07090f`}}>
      <div style={{width:"100%",maxWidth:400}} className="up">
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:11,marginBottom:8}}>
            <div style={{width:42,height:42,borderRadius:13,background:C.grad,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,fontWeight:800,boxShadow:"0 4px 20px #00d4ff30"}}>N</div>
        ...
        <Card>
          <div style={{display:"flex",background:C.surface,borderRadius:9,padding:3,marginBottom:22,border:`1px solid ${C.border}`}}>
            {[["login","Sign In"],["register","Register"]].map(([m,l])=>(
              <button key={m} onClick={()=>{setMode(m);setApiErr("");setErrs({});}}
                style={{flex:1,padding:"8px",borderRadius:7,border:"none",cursor:"pointer",fontWeight:600,fontSize:13,transition:"all .18s",background:mode===m?C.card:"transparent",color:mode===m?C.text:C.sub}}>
                {l}
              </button>
            ))}
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:14}} onKeyDown={e=>e.key==="Enter"&&submit()}>
            ...
            {mode==="register"&&<Input label="REFERRAL CODE (OPTIONAL)" placeholder="e.g. JOHN4x9z" value={form.referral_code} onChange={set("referral_code")}/>}
            {apiErr&&<div style={{background:C.redBg,border:`1px solid #f43f5e30`,borderRadius:8,padding:"10px 13px",fontSize:13,color:C.red}}>{apiErr}</div>}
...
const WithdrawModal = ({ balance, onClose, onSuccess, showToast }) => {
  ...
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()}
      style={{position:"fixed",inset:0,background:"#000000bb",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <Card style={{width:"100%",maxWidth:410,animation:"up .22s ease"}}>
        ...
        <div style={{background:C.accentBg,border:`1px solid ${C.accentBorder}`,borderRadius:8,padding:"9px 13px",fontSize:13,color:C.accent,marginBottom:16}}>
          Available: <strong>${Number(balance).toFixed(2)}</strong> · Min: $5.00
        </div>
  ...
  const referralLink=referralCode?`${window.location.origin}${window.location.pathname}?ref=${referralCode}`:"";
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg}}>
      ...
      <header style={{borderBottom:`1px solid ${C.border}`,background:C.surface,padding:"12px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
          <div style={{width:33,height:33,borderRadius:9,background:C.grad,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:17}}>N</div>
          <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <Badge label={rank} color={RANK_COLOR[rank]||C.accent}/>
          <div style={{fontSize:13,fontWeight:600,maxWidth:120,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user?.full_name||user?.name||"User"}</div>
      ...
      <main style={{maxWidth:940,margin:"0 auto",padding:"22px 14px"}}>
        <Card style={{marginBottom:20,borderColor:C.accentBorder,background:`linear-gradient(135deg,${C.accentBg},${C.card})`}} className="up">
          <div style={{marginBottom:10,display:"flex",flexWrap:"wrap",gap:8,justifyContent:"space-between",alignItems:"center"}}>
            ...
            {membershipId&&<div style={{background:C.goldBg,border:`1px solid ${C.goldBorder}`,borderRadius:7,padding:"4px 11px",fontSize:11,color:C.gold,fontFamily:"'Fira Code',monospace"}}>ID: {membershipId}</div>}
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
            <div style={{flex:1,minWidth:160,background:C.bg,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 12px",fontFamily:"'Fira Code',monospace",fontSize:12,color:C.accent,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{referralLink||"—"}</div>
        ...
        <div style={{display:"flex",gap:2,marginBottom:20,overflowX:"auto",paddingBottom:2,borderBottom:`1px solid ${C.border}`}}>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{padding:"9px 15px",border:"none",borderBottom:`2px solid ${tab===t.id?C.accent:"transparent"}`,cursor:"pointer",fontWeight:600,fontSize:12,whiteSpace:"nowrap",transition:"all .15s",background:"transparent",color:tab===t.id?C.accent:C.sub}}>
              {t.label}
            </button>
          ))}
        </div>
          ...
          tab==="overview"?(
            <div>
              <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:18}}>
                ...
                <Stat className="up3" label="CURRENT RANK"       value={rank}                color={RANK_COLOR[rank]||C.accent} icon="🏅"/>
              </div>
              {nextRank&&<Card style={{marginBottom:16,borderColor:`${C.gold}25`,background:C.goldBg}} className="up1"><div style={{fontSize:10,color:C.gold,fontFamily:"'Fira Code',monospace",marginBottom:3}}>NEXT RANK</div><div style={{fontSize:13}}>Keep referring to unlock <strong style={{color:C.gold}}>{nextRank}</strong></div></Card>}
              ...
              {commissions.length===0?<EmptyState icon="📋" title="No commissions yet" sub="Share your link to start earning!"/>:
                <Card style={{padding:0,overflow:"hidden"}}><Table heads={["From","Level","Amount","Date"]} rows={commissions.map(c=>[<span style={{fontWeight:500}}>{c.from_name||c.from||"—"}</span>,<Badge label={`Level ${c.level||1}`} color={c.level===2?C.green:C.accent}/>,<span style={{color:C.gold,fontWeight:700}}>{fmtMoney(c.amount)}</span>,<span style={{color:C.sub}}>{fmtDate(c.created_at)}</span>])}/></Card>}
            </div>
          ):tab==="referrals"?(
            <div className="up">
              <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:18}}>
                <Card style={{flex:1,minWidth:130,borderColor:C.accentBorder,background:C.accentBg}}><div style={{fontSize:10,color:C.accent,fontFamily:"'Fira Code',monospace",marginBottom:6}}>LEVEL 1 — DIRECT</div><div style={{fontSize:32,fontWeight:800,color:C.accent}}>{directCount}</div><div style={{fontSize:12,color:C.sub,marginTop:4}}>You referred these members</div></Card>
                <Card style={{flex:1,minWidth:130,borderColor:`${C.green}25`,background:C.greenBg}}><div style={{fontSize:10,color:C.green,fontFamily:"'Fira Code',monospace",marginBottom:6}}>LEVEL 2 — INDIRECT</div><div style={{fontSize:32,fontWeight:800,color:C.green}}>{indirectCount}</div><div style={{fontSize:12,color:C.sub,marginTop:4}}>Referred by your referrals</div></Card>
              </div>
            ...
            <div className="up">
              ...
              {commissions.length===0?<EmptyState icon="💎" title="No earnings yet" sub="Refer members to earn Level 1 & Level 2 commissions"/>:
                <Card style={{padding:0,overflow:"hidden"}}><Table heads={["From","Level","Amount","Status","Date"]} rows={commissions.map(c=>[<span style={{fontWeight:500}}>{c.from_name||c.from||"—"}</span>,<Badge label={`Level ${c.level||1}`} color={c.level===2?C.green:C.accent}/>,<span style={{color:C.gold,fontWeight:700}}>{fmtMoney(c.amount)}</span>,<Badge label={c.status||"paid"} color={C.green}/>,<span style={{color:C.sub}}>{fmtDate(c.created_at)}</span>])}/></Card>}
            </div>
          ):tab==="withdrawals"?(
            <div className="up">
              ...
              {withdrawals.length===0?<EmptyState icon="📤" title="No withdrawals yet" sub="Minimum withdrawal is $5.00" action={<Btn variant="gold" onClick={()=>setShowWithdraw(true)}>Request Withdrawal</Btn>}/>:
                <Card style={{padding:0,overflow:"hidden"}}><Table heads={["Amount","Bank","Account No.","Account Name","Status","Date"]} rows={withdrawals.map(w=>{const s=w.status||"pending";const sc=s==="approved"?C.green:s==="rejected"?C.red:C.gold;return[<span style={{color:C.gold,fontWeight:700}}>{fmtMoney(w.amount)}</span>,w.bank_name||"—",<span style={{fontFamily:"'Fira Code',monospace",fontSize:12}}>{w.account_number||"—"}</span>,w.account_name||"—",<Badge label={s} color={sc}/>,<span style={{color:C.sub}}>{fmtDate(w.created_at)}</span>];})}/></Card>}
            </div>
          ):(
            <div className="up">
              {notifications.length===0?<EmptyState icon="🔔" title="No notifications" sub="You're all caught up"/>:
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  {notifications.map((n,i)=>{const read=n.read||n.is_read;return(
                    <Card key={i} style={{borderColor:read?C.border:C.accentBorder,background:read?C.card:C.accentBg}}>
                      <div style={{display:"flex",gap:10,alignItems:"flex-start"}}>
                        <div style={{width:7,height:7,borderRadius:"50%",marginTop:5,flexShrink:0,background:read?C.border:C.accent}}/>
                        <div style={{flex:1}}>
                          <div style={{fontSize:14,fontWeight:read?400:600,marginBottom:4,lineHeight:1.4}}>{n.message||n.title||"Notification"}</div>
                          <div style={{fontSize:11,color:C.sub,fontFamily:"'Fira Code',monospace"}}>{fmtDate(n.created_at)}</div>
                        </div>
                      </div>
                    </Card>
                  );})}
...
export default function App() {
  ...
  return (
    <>
      <style>{css}</style>
      {user?<Dashboard user={user} onLogout={logout}/>:<AuthScreen onAuth={u=>setUser(u)}/>}
    </>
  );
}
